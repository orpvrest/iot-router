...
1. Скопируйте `.env.example` в `.env` и задайте значения:
   ...
   - `VPN_FORWARD_TCP` — определения проброса портов `label:public_port:client_name:client_port` (через запятую). Публичный порт сразу слушается на хосте (контейнер запускает `socat`), поэтому достаточно указать номер.
   ...
   - `PORT_FORWARD_BIND_ADDR` — адрес, на котором `socat` внутри `openvpn-core` слушает проброшенные TCP-порты. Для `network_mode: host` используйте `127.0.0.1`, чтобы порты были доступны только через `nginx-edge`/сторонние прокси; для bridge-режима оставьте `0.0.0.0`.
...
- Дополнительные TCP порты из `VPN_FORWARD_TCP` публикуются напрямую (через `socat`), nginx больше не занимается их проксированием.
...
1. Выберите свободный внешний порт (например `20022` или `25900`) — он будет слушаться на сервере напрямую.
...
> Если нужен конкретный «нестандартный» порт (например 22), просто добавьте его в `VPN_FORWARD_TCP` — `socat` откроет его напрямую на хосте.
